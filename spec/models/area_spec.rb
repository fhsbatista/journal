...
RSpec.describe Area, type: :model do
  ...
  describe '#day_score' do
    ...
    let!(:event2) { Event.create(behavior: behavior2, created_at: Date.current) }
    let!(:event3) { Event.create(behavior: behavior3, created_at: Date.current) }
    ...
    it 'returns the total score of all today events for all behaviors' do
      expect(area.day_score(Date.current)).to eq(12.0) # 10.0 + 2.0
    end
  end
  ...
  describe '#areas day average' do
    ...
    let!(:event4) { Event.create(behavior: exercises, created_at: Date.current) }
    let!(:event5) { Event.create(behavior: eat_clean, created_at: Date.current) }
    let!(:event6) { Event.create(behavior: conciliation, created_at: Date.current) }
    let!(:event7) { Event.create(behavior: pay_bills, created_at: Date.current) }
    let!(:event8) { Event.create(behavior: pomodoros, created_at: Date.current) }
    ...
    it 'returns score average of a given date' do
      expect(Area.average(Date.yesterday)).to eq(2)
      expect(Area.average(Date.current)).to eq(3.667)
    end
  end

  describe '#areas 7-day average' do
    let!(:health) { Area.create(description: 'Health') }
    let!(:finances) { Area.create(description: 'Finances') }
    let!(:work) { Area.create(description: 'Work') }
    let!(:exercises) { Behavior.create(description: 'Exercices', area: health) }
    ...
    it 'returns past x days score average of a given date' do
      expect(Area.days_average(since: Date.yesterday, days: 7)).to eq(0.286)
      expect(Area.days_average(since: Date.current, days: 7)).to eq(0.810)
      expect(Area.days_average(since: Date.current, days: 2)).to eq(2.834)
    end
  end
end
